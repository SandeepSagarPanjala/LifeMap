...
describe('database migrations', () => {
  it('loads bundled sqlite migrations in journal order', () => {
    ...
    expect(prepared).toHaveLength(45);
    ...
    expect(prepared[33]?.tag).toBe('0033_visit_label_override_anchor');
    expect(prepared[33]?.sql.join('\n')).toContain('anchor_lat');
    expect(prepared[34]?.tag).toBe('0034_moment_thumbnail_path');
    expect(prepared[34]?.sql.join('\n')).toContain('thumbnail_path');
    expect(prepared[35]?.tag).toBe('0035_moment_tags_json');
    expect(prepared[35]?.sql.join('\n')).toContain('tags_json');
    expect(prepared[36]?.tag).toBe('0036_activity_definitions');
    expect(prepared[36]?.sql.join('\n')).toContain('definition_json');
    expect(prepared[36]?.sql.join('\n')).toContain('activity_values_json');
    expect(prepared[37]?.tag).toBe('0037_moment_mood_reason_variant');
    expect(prepared[38]?.tag).toBe('0038_moment_type_mood_voice_transcript');
    expect(prepared[39]?.tag).toBe('0039_activity_reminders');
    expect(prepared[39]?.sql.join('\n')).toContain('reminder_enabled');
    ...
    expect(prepared[44]?.tag).toBe('0044_schema_trim');
    expect(prepared[44]?.sql.join('\n')).toContain('tracking_events');
  });
