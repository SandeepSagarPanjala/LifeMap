...
function poi(
  overrides: Partial<PlacePoiRow> & Pick<PlacePoiRow, 'id' | 'name' | 'lat' | 'lng'>,
): PlacePoiRow {
  return {
    cacheId: 1,
    source: 'mapkit',
...
describe('poi coordinate refresh helpers', () => {
  it('detects POIs at the geocode anchor', () => {
    ...
    expect(poiCoordinatesMatchAnchor({lat: 33.22, lng: -97.16}, anchor)).toBe(true);
    expect(poiCoordinatesMatchAnchor({lat: 33.23, lng: -97.16}, anchor)).toBe(false);
  });
