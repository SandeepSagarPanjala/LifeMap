...
describe('location_points dedupe maintenance', () => {
  function createSqlite(rows: Array<Record<string, unknown>> = []) {
    ...
    return {
      execute: jest.fn(async (query: string, params?: unknown[]) => {
        ...
        if (normalized.includes('FROM sqlite_master') && params?.[0] === LOCATION_POINTS_DEDUPE_UNIQUE_INDEX) {
          return {rows: indexes.has(LOCATION_POINTS_DEDUPE_UNIQUE_INDEX) ? [{name: LOCATION_POINTS_DEDUPE_UNIQUE_INDEX}] : []};
        }
        if (normalized.includes('FROM sqlite_master') && params?.[0] === 'location_points_no_delete') {
          return {rows: triggers.has('location_points_no_delete') ? [{name: 'location_points_no_delete'}] : []};
        }
        if (normalized.startsWith('SELECT COUNT(*) AS count FROM location_points')) {
        ...
        if (normalized.includes('HAVING group_count > 1')) {
          ...
          const extras = [...groups.values()].filter(count => count > 1).map(count => count - 1);
        ...
        if (normalized.startsWith('DELETE FROM location_points')) {
          ...
          locationRows = locationRows.filter(row => keepIds.has(Number(row.id)));
          return {rows: []};
        }
        return {rows: []};
      }),
  ...
  it('counts duplicate extra rows', async () => {
    ...
    await expect(countLocationPointDuplicateExtraRows(sqlite as never)).resolves.toBe(1);
    await expect(countLocationPointDuplicateGroups(sqlite as never)).resolves.toBe(1);
  });
  ...
  it('creates the unique index when no duplicates remain', async () => {
    ...
    await expect(ensureLocationPointsDedupeUniqueIndex(sqlite as never)).resolves.toBe(true);
    await expect(ensureLocationPointsDedupeUniqueIndex(sqlite as never)).resolves.toBe(false);
  });
