    ...
    return (
      <View style={{ width }} className="flex-1 justify-end px-6 pb-3">
        <View
          ...
          style={
            item.id === 'capture-moments' ? { marginBottom: 42 } : undefined
          }
        >
          {item.lottie ? (
            <LottieView
            ...
            <View className="mt-4 gap-3">
              {item.bullets.map(bullet => (
                <View key={bullet} className="flex-row gap-3">
                  <Text className="text-primary mt-0.5 text-lg leading-6">
                    •
                  </Text>
                  <Text
                    variant="muted"
                    className="text-muted-foreground flex-1 text-base leading-6"
                  >
                    {bullet}
                  </Text>
                </View>
